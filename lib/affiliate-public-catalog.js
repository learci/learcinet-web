...
function normalizeOffer(value) {
  ...
  return VALID_OFFERS.has(offer) ? offer : "all";
}

function normalizeSort(value) {
  const sort = normalizePublicFilterString(value) || "recommended";
  return VALID_SORTS.has(sort) ? sort : "recommended";
}
...
if (requestedOffer !== "all") {
  url.searchParams.set("offer", requestedOffer);
}

if (requestedSort !== "recommended") {
  url.searchParams.set("sort", requestedSort);
}
