...
export default function ShapeGrid({
  ...
  useEffect(() => {
    ...
    const rememberHover = (next) => {
      if (
        !hoveredSquare.current ||
        hoveredSquare.current.x !== next.x ||
        hoveredSquare.current.y !== next.y
      ) {
  ...
  return (
    <canvas
      ref={canvasRef}
      className={`shapegrid-canvas ${className}`.trim()}
      aria-hidden="true"
    />
  );
}
